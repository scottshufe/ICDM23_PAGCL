...
from priv_weights import shortest_path_weights, common_neighbor_weights, combine_weights, \
    combine_weights_v2, dice_aug, edge_rand, lap_graph, get_priv_adj, get_weight_matrix
...
def train():
    ...
    def drop_edge(idx: int):
        ...
        if param['drop_scheme'] == 'uniform':
            ...
            print("total delete: {}, sp delete: {}, other delete: {}".format(t_delete / 2, sp_delete / 2,
                                                                             other_delete / 2))
            print(edges.shape)
            return edges
...
def scenario_II_link_eval(epoch):
    ...
    for cls_name in ['lr', 'xgb', 'mlp']:
        for op in ['hadamard', 'l1', 'l2', 'avg', 'concat']:
            ...
            with open('{}/scenario2_files/{}_{}_{}_scenario2_results.txt'.format(final_folder, param['drop_scheme'],
                                                                                 cls_name, op), 'a') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='LastFMAsia')
    ...
    final_folder = 'attacks_{}_output_files'.format(args.dataset)
    ...
    for iter_num in range(1, 6):
        ...
        if param['drop_scheme'] == 'degree':
            ...
            if param['drop_scheme'] == 'shortest_path':
                ...
                drop_weights = shortest_path_weights(data, test_data, strategy='simple', rm_edge_num=3,
        ...
        for epoch in range(1, param['num_epochs'] + 1):
            ...
            if (epoch) % 100 == 0:
                ...
                #            '{}/tmp_emb_files/iter{}_epoch{}_{}_model.pt'.format(final_folder, iter_num, epoch,
                #                                                                 param['drop_scheme']))
                ...
                #     metric = sim_list_str[i]
                #     s1_auc = get_sim_auc(i, sim_list, test_edge_labels)

                sim_metric, s1_auc = write_auc(args.dataset, epoch, "{}/scenario1_files".format(final_folder), sim_list,
                                               test_edge_labels)
                # s1_auc, s1_ap_score = link_eval()

                # machine learning based attacks
                scenario_II_link_eval(epoch)

                print(f'(E) | Epoch={epoch:04d}, avg_acc = {acc}, link prediction auc = {s1_auc}')
                with open('{}/scenario1_files/{}_results.txt'.format(final_folder, param['drop_scheme']), 'a') as file:
                    file.write(f'{epoch}\t{acc}\t{sim_metric}\t{s1_auc}\t\n')
                    file.flush()
